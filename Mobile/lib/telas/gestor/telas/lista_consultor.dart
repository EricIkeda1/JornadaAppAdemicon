...
class _ConsultoresRootState extends State<ConsultoresRoot> {
  ...
  Future<void> _openEditarConsultor(_ConsultorView c) async {
    final changed = await showDialog<bool>(
      ...
      builder: (ctx) => _EditarConsultorDialog(
        consultor: c,
        phoneFmt: _phoneFmt,
      ),
    );
  ...
  Future<void> _confirmarExcluir(_ConsultorView c) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Conta marcada como inativa'),
...
class _ToggleAtivos extends StatelessWidget {
  ...
  const _ToggleAtivos(
      {super.key, required this.ativos, required this.onChanged});

  @override
  Widget build(BuildContext context) {
    const borderColor = Color(0xFFE3E3E6);
    final Color pillColor =
        ativos ? const Color(0x1A7A1315) : const Color(0x1A939598);
    return Material(
      color: Colors.white,
        ...
        child: Stack(
          ...
          children: [
            AnimatedAlign(
              ...
              alignment:
                  ativos ? Alignment.centerLeft : Alignment.centerRight,
...
class _StatusButtonOutlined extends StatelessWidget {
  ...
  final bool ativoTheme;
  const _StatusButtonOutlined(
      {required this.onPressed, required this.ativoTheme});
  ...
  Widget build(BuildContext context) {
    ...
    return ConstrainedBox(
      constraints:
          const BoxConstraints.tightFor(height: 36, width: 92),
...
class _NomeMatricula extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final styleNome = TextStyle(
      ...
      decoration:
          ativo ? TextDecoration.none : TextDecoration.lineThrough,
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'Mat. ${matricula.isEmpty ? '—' : matricula}',
          style: const TextStyle(
            fontSize: 12,
      ...
      style: OutlinedButton.styleFrom(
        ...
        ButtonStyle(
          overlayColor: MaterialStateProperty.resolveWith(
            (s) => brandDeepRed.withOpacity(
              s.contains(MaterialState.pressed) ? 0.12 : 0.08,
            ),
          ),
        ),
      ),
...
class _EditarConsultorDialog extends StatefulWidget {
  ...
  const _EditarConsultorDialog({
    required this.consultor,
    required this.phoneFmt,
  });

  @override
  State<_EditarConsultorDialog> createState() =>
      _EditarConsultorDialogState();
}
...
class _EditarConsultorDialogState extends State<_EditarConsultorDialog> {
  ...
  void initState() {
    ...
    _telCtrl = TextEditingController(
      text: fmt.formatStatic(widget.consultor.telefone),
    );
    _emailCtrl = TextEditingController(text: widget.consultor.email);
    _matCtrl = TextEditingController(
      text: widget.consultor.matricula.isEmpty
          ? ''
          : widget.consultor.matricula,
    );
  }
  ...
  InputDecoration _dec(String hint) => InputDecoration(
        ...
        focusedBorder: const OutlineInputBorder(
          ...
          borderSide: BorderSide(color: brandDeepRed),
        ),
      );
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await _client.from('consultores').update({
        ...
        'matricula':
            _matCtrl.text.trim().isEmpty ? null : _matCtrl.text.trim(),
            ...
            child: Column(
              ...
              children: [
                ...
                _BannerBox(
                  ...
                  title:
                      'Você já transferiu os leads deste consultor?',
                  subtitle:
                      'Certifique-se de que todos os leads foram transferidos antes de excluir.',
